...
| Os três hooks de `.claude/hooks/` comparavam caminho distinguindo caixa, contra um NTFS que não distingue: `server_V2.py`, `mcp/Tests/t.py`, `MCP/TESTS/t.py` e `a.PY` escapavam todos. Em `proteger-oraculo.sh` isso furava a proteção do oráculo durante missão ativa, que é a integridade que ele existe para garantir | `re.IGNORECASE` em `impedir-copia.sh`; `tr 'A-Z' 'a-z'` na cópia de comparação dos outros dois, nunca no caminho que vai para a ação. É o **inverso** do caso `-cnotcontains` acima: lá o filesystem distinguia caixa e a comparação não; aqui o filesystem não distingue e a comparação distinguia. O princípio é o mesmo nos dois — a comparação segue a semântica do filesystem | `.claude/hooks/test-impedir-copia.sh`, `test-proteger-oraculo.sh` e `test-formatar-python.sh`, 47 casos, RED medido antes de cada conserto. **Não** rodam na CI: a matriz ubuntu distingue caixa e os casos de caixa seriam falsos lá |
